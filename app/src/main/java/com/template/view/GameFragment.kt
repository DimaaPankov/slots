    ...
    override fun clickSpin() {
        binding.Bspin.setOnClickListener {
            ...
            binding.betCount.text = presenter.getBet().toString()
             binding.betCount.setTextColor(ContextCompat.getColor(requireContext(), R.color.white))

            if ((presenter.getMainCont().toInt() > 0) or (presenter.getBet() < presenter.getMainCont().toInt())) {
                    ...
                    binding.betCount.text = presenter.getMainCont()
                presenter.win {
                    Handler(Looper.getMainLooper()).postDelayed({
                        presenter.setMainCount(presenter.winBet,true)
                        val directions = GameFragmentDirections.actionGameFragmentToAnimationVinFragment(
                            arrayOf(
                                presenter.getSlots()[0], presenter.getSlots()[1], presenter.getSlots()[2],
                                presenter.getSlots()[3], presenter.getSlots()[4], presenter.getSlots()[5],
                                presenter.getSlots()[6], presenter.getSlots()[7], presenter.getSlots()[8],
                            )  /* presenter.getSlots()*/, presenter.betCountXXX, presenter.winBet,
                            booleanArrayOf(presenter.winLine1, presenter.winLine2, presenter.winLine3)
                        )
